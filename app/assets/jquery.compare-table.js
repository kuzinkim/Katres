...
;(function ($) {
	...
	function CompareTable(options) {
		this.inited = options.inited || false;                      // инициализация
		this.columns = options.columns || 1;                        // колличество видимых колонок свойств
		this.lines = options.lines || 10;                           // колличество видимых линий
		this.folding = options.folding || false;                    // сворачивание таблички
		this.pagination = options.pagination || false;              // отображение пагинации
		this.showArrows = options.showArrows || true;               // отображение стрелок
		this.prev = options.prev || '<';                            // стрелка влево
		this.next = options.next || '>';                            // стрелка вправо
		this.sort = options.sort || false;                          // сортировка таблицы
		this.sortIcon = options.sortIcon || '<';                    // картинка сортировки
		this.duplicate = options.duplicate || false;                // отображение дублированных колонок
		this.headerFixed = options.headerFixed || false;            // фиксированная шапка таблицы
		this.responsive = options.responsive || false;              // адаптивность
	}
	...
	CompareTable.prototype.addPagination = function() {
		...
		if (qtyPages < 2) {
			...
			if (pagination[0]) {
				...
				htmlPagination += '<div class="tpaging"><button class="tpaging__more button button_more" data-value="more"><span>Показать еще</span><svg width="20" height="20"><use xlink:href="/upload/images/sprite.svg#arrow"></use></svg></button><div class="tpaging__pages">' + htmlPages + '</div></div>';
	...
	CompareTable.prototype.addFolding = function() {
		...
		let pagination = this.table.find('.tpaging');
		let htmlControlFolding = '<div class="tpaging tpaging_fold"><button class="tpaging__fold folded" data-value="more"><span>Развернуть</span><span>Свернуть</span><svg width="20" height="20"><use xlink:href="/upload/images/sprite.svg#arrow"></use></svg></button></div>';
	...
	CompareTable.prototype.getValue = function(value) {
		let textValue = value.trim().replace(/\s+(?!euro)/g,"").toLowerCase(); // удаляем пробелы и приводим к нижнему регистру

		if (textValue.length < 1) {
			return ' ';
		}

		let numValue = textValue.replace(/[,]/g,".") // "," на "."
														.replace(/[^.-\d]+/g,""); // оставить только цифры, точки и знаки минуса

		let reg_exp_1 = textValue.match(/^[0-9\-]|^(от)/); // в начале цифра, знак '-' или 'от'
		let reg_exp_2 = textValue.match(/;|\!|\(|\)/); // знаки ';', '!', '(', ')'
		let reg_exp_3 = numValue.match(/\./g); // точки
